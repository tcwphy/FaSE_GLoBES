...
In addition, the true theory that explains the origin of neutrino mass is waiting to be {\color{blue}found out}. \jt{[Not clear to me.]}
...
One of these models, {\color{blue}models based on} the seesaw mechanism has been used to explain such a tiny mass in the neutrino sector.\jt{[Seesaw mechanism is not a model, isn't it?]}
...
We compute the minimal $\chi^2$ value for the model, and allowed all model parameters varied \jt{within} the prior \jt{defined} in Eq.~(\ref{eq:prior}) according to NuFit4.0 results.
In addition, the studied statistics function is exactly given by Eq.~(\ref{eq:chi-squared}), {\color{blue} the true event rate $n_i$ is predicted by a set of assumed oscillation parameters. Two parameters $\theta_{23}$ and $\delta$ in $\vec{\theta}^{true}_{OSC}$ keep varied in the range of $40^\circ<\theta_{23}<53^\circ$ and $125^\circ<\delta<390^\circ$, respectively.} \jt{Explanations are note clear to me???}
More details are presented in the user manual\footnote{The manual is available on the \textbf{FaSE} repository  \url{https://github.com/tcwphy/FASE_GLoBES.git}.}. \jt{(Website to download the manual???)}
