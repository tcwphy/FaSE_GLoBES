...
\textbf{FaSE-GloBES} can be used to study how the model parameter is constrained by simulated neutrino oscillation experiments. We take \textit{the tri-direct littlest seesaw} (TDLS)~\cite{King:2013iva,King:2015dvf,King:2016yvg} as an example. Under this model, the light left-handed Majorana neutrino mass matrix is given by
\begin{equation}
...
{\color{red}To show the generality of \textbf{FaSE-GLoBES}, we present the similar result with another model -- \textit{the warped flavor symmetry} (WFS). This model predict the simplification that mixing angles and the CP phase are functions of two model parameters $\theta_\nu$ and $\phi_\nu$,
\begin{align}
