...
We take TDLS as an example model, to see how the model parameter can be constrained by MOMENT experiment. To get the constraint of $x$ and $\eta$, we obtain the minimum of $\chi^2$ value as the following script in the main code.\vspace{0.2cm}\\
\texttt{
...
The output is a set of $\Delta \chi^2(\vec{\theta}^{hyp})$ (the third column) values with the changed $x$ (the first column), $\eta$ (the second column) in the file \texttt{data/contraint\_x\_eta.dat}. We use these data, and find the $1\sigma$, $2\sigma$, and $3\sigma$ constraints of $x$ and $\eta$ shown in Fig.~\ref{fig:x_eta}. 
